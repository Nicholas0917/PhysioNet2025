...
class Config:
    def __init__(self):
        ...
        self.num_epochs = 100
        self.learning_rate = 5e-4  # Increased learning rate
...
def train_model(data_folder, model_folder, verbose):
    ...
    # Second stage: freeze all except dense layer
    if len(finetune_records) > 0:
        for name, param in model.named_parameters():
            if 'dense' in name:  # Only update dense layer
                param.requires_grad = True
            else:
    ...
    # Initialize SWA for fine-tuning
    swa_model = torch.optim.swa_utils.AveragedModel(model)
    swa_start = int(num_epochs * 0.75)  # Start SWA after 75% of training
...
class HybridModel(nn.Module):
    def __init__(self, device, pth_path):
        ...
        self.base_model = Net1D(
            ...
            use_bn=False,
            use_do=False,
            n_classes=1  # Temporary value, will be replaced
        )
        ...
        # Replace classification head with single output
        in_features = self.base_model.dense.in_features
        self.base_model.dense = nn.Linear(in_features, 1).to(device)
        
        # Initialize new dense layer with kaiming
        nn.init.kaiming_normal_(self.base_model.dense.weight, mode='fan_in', nonlinearity='linear')
        nn.init.zeros_(self.base_model.dense.bias)
