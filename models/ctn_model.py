# https://arxiv.org/abs/2411.00755
import math
# import timm
import torch
...
from models.attn_net_gated import *
# from patch_dropout import *
from typing import Optional, Any, Union, Callable
import torch
from torch import Tensor
from models.multihead_diffattn import *
...
class TransformerEncoderLayerWithDiffAttn(nn.TransformerEncoderLayer):
    def __init__(self, d_model, nhead, dim_feedforward=2048, dropout=0.1, depth=1, activation="relu"):
        print(f"DEBUG: d_model={d_model}, nhead={nhead}")
...
class CTN(nn.Module):
    def __init__(self, d_model, nhead, d_ff, num_layers, dropout_rate, deepfeat_sz, nb_feats, nb_demo, classes, class_token,\
        ...
        if self.if_attn_gated_module:
            self.global_attn_pool = Attn_Net_Gated(L=114, D=114, dropout=0.25, n_classes=len(classes)) #changed from 184 -> 180 -> 114
            self.global_rho = nn.Sequential(*[nn.Linear(114, 114), nn.ReLU(), nn.Dropout(0.25)])    #changed from 184 -> 180 -> 114
            #exp 9
            ...
            self.wide_project = nn.Sequential(
                ...
                nn.Linear(92, 114)  # Second linear layer to output 114 (matching sequence length)
            )
            bag_classifiers = [nn.Linear(114, 1) for i in range(len(classes))] #use an indepdent linear layer to predict each class
    ...
    def forward(self, x, wide_feats):
        ...
        for i, layer in enumerate(self.encoder):
            ...
            if isinstance(layer, nn.Conv1d):
                ...
                # print(f"After Conv1d layer {len(intermediate_embeds)}: {z.shape}")
        
        # print(f"Features shapes: {[f.shape for f in intermediate_embeds]}")
        # out = self.transformer(z)    # transformer output is batch_sz x d_model
        cls_token,out = self.transformer(intermediate_embeds)
        # print(f"Transformer output shape: {out.shape}")
        # print(f"Final sequence length: {out.shape[0] if len(out.shape) > 2 else out.shape[-1]}")

        # attn_gated_module
        if self.if_attn_gated_module: ### Global Aggregation
            # exp9:
            ...
            # exp11:
            out = out.permute(1,2,0)
            wide_feats = self.wide_project(wide_feats.unsqueeze(2)) # B,22->B,22,114
            out = torch.cat([wide_feats, out], dim=1) #B,264+22,114
            A_4096, h_4096 = self.global_attn_pool(out)  # N, n_classes ;  N,D,
            A_4096 = torch.transpose(A_4096, 2, 1)
            A_4096 = F.softmax(A_4096, dim=2) 
            h_path = torch.bmm(A_4096, h_4096) #B,n_classes,114(N_sequence)
            out = self.global_rho(h_path) 
...
def create_ctn_model(config, device):
    model = CTN(
        ...
        nb_feats=config.nb_feats,
        nb_demo=config.nb_demo,
        classes=config.classes,
        class_token=config.class_token,
        if_attn_gated_module=config.if_attn_gated_module
    )
