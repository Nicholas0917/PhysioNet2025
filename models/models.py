...
from models.ctn_model import CTN, create_ctn_model
...
class HybridModel(nn.Module):
    def __init__(self, device, pth_path, config=None):
        ...
        elif self.base_model_name == 'CTN':
            self.base_model = create_ctn_model(self.config, device)
            base_model_output_dim = self.config.deepfeat_sz  # or whatever CTN outputs before classifier
    ...
    def forward(self, x, meta_features=None):
        ...
        if self.base_model_name == 'CTN':
            #CTN handles prediciton internally
            return self.base_model(x, meta_features)
