...
def run_model(record, model, verbose):
    ...
    signal = np.load(record.replace('.hea', '_img.npy'))
    meta_features = extract_features(record)[1]
...
def datapreprocess(records):
    for record in records:
        ...
        for lead in range(signal.shape[1]):
            coeffs, freqs = pywt.cwt(signal[:, lead], scales=np.arange(1, 129), wavelet='morl')
            img = np.abs(coeffs)
            img = cv2.resize(img, (224, 224))
            imgs.append(img)
        imgs = np.stack(imgs, axis=0)
        np.save(record.replace('.hea', '_img.npy'), imgs)
...
class ECGDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        imgs = np.load(record.replace('.hea', '_img.npy'))
        meta_features = extract_features(record)[1]
        label = float(load_label(record))
        return [imgs, meta_features], label
...
################################################################################
#
# Pretrained model using timm library
#
################################################################################
...
class PretrainedModel(nn.Module):
    def __init__(self, model_name='convnext', pretrained=False, out_channel=1, dropout_rate=0.2):
        super(PretrainedModel, self).__init__()
        self.model = timm.create_model(model_name, pretrained=pretrained, in_chans=12, features_only=True)
        self.fc1 = nn.Linear(config.get_meta_feature_dim(), 32)
        self.dropout = nn.Dropout(dropout_rate)
        self.new_head = nn.Linear(768 + 32, out_channel)
        nn.init.kaiming_normal_(self.new_head.weight, mode='fan_out', nonlinearity='relu')

    def forward(self, x, meta_feature):
        # x.shape [b, 12, h, w], meta_feature.shape [b, n]
        x = self.model(x)
        x = x[-1]  # Assuming the last feature map is the one we need
        x = F.adaptive_avg_pool2d(x, (1, 1)).view(x.size(0), -1)
        # print(x.shape)
        meta_feature = F.gelu(self.fc1(meta_feature))
        meta_feature = self.dropout(meta_feature)
        x = torch.cat((meta_feature, x), dim=1)
        x = self.new_head(x).squeeze()
        return x